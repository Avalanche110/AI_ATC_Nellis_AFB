...
AI_ATC.Radio = {
  ...
  ["SFA"] =      {    Key = "Departure",    UHF = 343.725, VHF = 124.95, UserFreq = "UHF", Transmitter = "Nellis_Approach" },
...
AI_ATC.Departure.SID = "FYTTR 7"
AI_ATC.Departure.VFR = "FYTTR"
...
AI_ATC_SoundFiles.Ground.Aircraft = {
  ...
  ["T-45C"] = { filename = "T45.ogg", duration = 0.923 },
...
function AI_ATC:ClearanceSubMenu(Alias)
  ...
  local function SetDeparture(Type)
    ...
    ClientData = ATM.ClientData[Alias]
    if ClientData then
      ...
      ClientData.DepartureCode      = departureData.Code
...
function AI_ATC:TakeOffSubMenu(Alias)
  ...
  if not Helo then
    ...
    local ClimbMenu1 = MENU_GROUP:New(Group, "5k to 14k", ClimbMenu)
    local ClimbMenu2 = MENU_GROUP:New(Group, "15k to 22k", ClimbMenu)
...
function AI_ATC:NATCF_SALLYSubMenu(Alias)
  ...
  if AI_ATC.Procedure=="IFR" and not Helo then
  ...
  elseif AI_ATC.Procedure=="VFR" or Helo then
...
function AI_ATC:ApproachTypeMenu(Alias)
  ...
  MENU_GROUP_COMMAND:New(Group, "Request GCA", AppraochMenu, function() AI_ATC:PushGCA(Alias) end, Group)
end
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--************************************************************************NELLIS GCA SUBMENU*************************************************************************************--
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
function AI_ATC:GCASubMenu(Alias, missed, modifier)
  ...
  if not modifier then
    MENU_GROUP_COMMAND:New(Group, "GCA Check in", AppraochMenu, function() AI_ATC:ApproachManager(Alias, "PAR", missed) end, Group)
  else
...
function AI_ATC:BackToRadarSubMenu(Alias)
  ...
  MENU_GROUP_COMMAND:New(Group, "Cancel IFR", AppraochMenu, function() AI_ATC:CancelIFR(Alias) end, Group)
  MENU_GROUP_COMMAND:New(Group, "Request GCA", AppraochMenu, function() AI_ATC:PushGCA(Alias, true, true) end, Group)
end
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--***********************************************************************APPROACH NAV ASSIST*************************************************************************************--
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
function AI_ATC:ApproachAssistMenu(Alias, report)
  ...
  if Procedure=="IFR" then
    MENU_GROUP_COMMAND:New(Group, "Cancel IFR", AppraochMenu, function() AI_ATC:ApproachCheckIn2(Alias, "VFR") end, Group)
  end
end
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--***********************************************************************APPROACH SUB MENU**************************************************************************************--
...
function AI_ATC:MissedApproachMenu(Alias)
  ...
  MENU_GROUP_COMMAND:New(Group, Procedure5, AppraochMenu, function()AI_ATC:PushGCA(Alias, true) end, Group)
end
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
function AI_ATC:LandingSubMenu(Alias, Modifier)
  ...
  if Helo~=true then
    ...
    MENU_GROUP_COMMAND:New(Group, "Straight In", CommandMenu, function() AI_ATC:RequestJollyPad(Alias) end, Group)
  end
...
function AI_ATC:InitialSubMenu(Alias)
  ...
  local InitalMenu = MENU_GROUP:New(Group, "Report 5 mile", CommandMenu)
...
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
function AI_ATC:NavpointSubMenu(Alias, Navpoint)
  local Group = ATM.ClientData[Alias].Unit:GetGroup()
  local Runway = AI_ATC.Runways.Landing[1]
  ...
  if not Navpoint then
    ...
    Subtitle = string.format("Report %s", Navpoint)
  end
...
function AI_ATC:SetCallsign(Alias, Callsign, Integer)
  ...
  local CurrentCallsign = GroupObject:GetCallsign()
  ...
  if AI_C2 and AI_C2.Instances then
    for _, c2 in pairs(AI_C2.Instances) do
      if c2 and c2.ClientData and c2.ClientData[Alias] then
        c2:SetCallsign(Alias, Callsign, Integer)
       end
     end
  end
  ...
  if AI_JTAC and AI_JTAC.Instances then
    for _, JTAC in pairs(AI_JTAC.Instances) do
      if JTAC and JTAC.ClientData and JTAC.ClientData[Alias] then
        JTAC:SetCallsign(Alias, Callsign, Integer)
       end
     end
  end
  ...
  SCHEDULER:New(nil, function()
    ...
    TowerRadar:ForEachGroup(function(GroupObject)
      ...
      if not ATM.TowerControl[Alias] then
        ...
        if Angle > 150 and Altitude <= 300 then
      ...
      elseif ATM.TowerControl[Alias] and GroupObject:IsAlive() and Range<=18520 then
        ...
        elseif ATM.TowerControl[Alias].State=="Going around" then
          ...
          if HeadingNo >= 30 and HeadingNo <= 180 then
            ATM.TowerControl[Alias].State = "Outside Downwind"
            return
          elseif HeadingNo >= 230 and HeadingNo <= 340 then
            ATM.TowerControl[Alias].State = "Missed Approach"
            return
          end
          
        elseif ATM.TowerControl[Alias].State=="Outside Downwind" then
          if Angle >= 80 and Angle <= 140 and Range >= 9260 then
...
function AI_ATC:ProvideInstruction(Alias, Agency, State)
  if AI_ATC.Instruction==true then
    ...
    local Squawk = ClientObject.Squawk
    ...
    local VA_Instruction = {
      ...
      ["Clearance"] = {
        ...
        ["ReadBack"] = {Callsign..", Squawk "..Squawk},
      },
      ["Tower"] = {
...
function AI_ATC:TaxiToParking(Alias, RadioObject, transmitter)
  ...
  local Title = string.format("%s %s", AI_ATC.Airbase, transmitter)
  local Unit = ATM.ClientData[Alias].Unit
  local UnitHeading = AI_ATC.Runways.Landing[5]
...
function AI_ATC:Vector(Alias, String)
  ...
  SCHEDULER:New( nil, function()
    if AI_Instructor and AI_Instructor.RequestVector==true then
      AI_Instructor.VectorGiven = true
    end
  ...
  SCHEDULER:New( nil, function()
    if Destination then
      local Coord = Unit:GetCoordinate()      
      local Heading = math.floor(((Coord:HeadingTo(Destination) - MagDec + 360) % 360 + 0.5 ))
...
function AI_ATC:NavAssist(Alias, NavPoint, Transmitter)
  ...
  if Transmitter=="Approach" and Type=="PAR" then
  ...
  SCHEDULER:New( nil, function()
    ...
    if NavPoint=="TACAN" then
      ...
      AI_ATC:Runway(Runway, RadioObject, Agency)
      ...
      AI_ATC:ReadNavpoint(NavPoint, RadioObject, Agency)
      RadioObject:NewTransmission("FlyHeading.ogg", 0.801, "Airbase_ATC/Departure/SoundFiles/", nil, 0.03)
    end
    AI_ATC:ReadHeading(Heading, RadioObject, Agency)
  end,{}, Delay)
end
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--***********************************************************************************ATIS START**********************************************************************************--
...
function AI_ATC:ClearanceDelivery(Alias, Type)
  ...
  local CallsignSub     = Flight and FlightCallsign or Callsign
  local Delay           = 1.5 + math.random() * (2.5 - 1.5)
  local departureData, instruction, war, squawk, firstDigit, Procedure, Sequential, subtitle1, Altitude, FlightLevel
  ...
  SCHEDULER:New(nil, function()
    AI_ATC:ProvideInstruction(Alias, "Clearance", "ReadBack")
  end,{}, 10 )
  ...
  SCHEDULER:New(nil, function()
    ...
    if Procedure=="IFR" then
      Altitude = AI_ATC.SID[Type].Altitude
      FlightLevel = AI_ATC:ReadFlightLevel(Altitude, RadioObject, Transmitter, false, nil)
    end
    ...
    if AI_Instructor then
      if AI_Instructor.Active == true then 
        SCHEDULER:New(nil, function()
          local Txt = "Use the F-10 Menu to Read back your clearance with the correct Squawk code."
          local Result = UTILS.OneLineSerialize(Txt)
          MESSAGE:New(Result, 9):ToAll()
        end, {}, Delay+5)
      end
    end
  ...
  SCHEDULER:New(nil, function()
    ...
    if Procedure=="IFR" then
      subtitle1 = string.format("%s: Fly runway heading to 5,000, expect %s 5 minutes after departure.", Title, FlightLevel)
      subtitle = subtitle1..string.format(" Departure frequency 385.4. Squawk %s", squawk)..Sequential
      RadioObject:NewTransmission("DeadAir.ogg", 0.100, "Airbase_ATC/Ground/SoundFiles/", nil, nil, subtitle, 10)
      RadioObject:NewTransmission("IFRClearance.ogg", 2.984, "Airbase_ATC/Clearance/SoundFiles/", nil, 0.05)
      AI_ATC:ReadFlightLevel(Altitude, RadioObject, Transmitter, true, 0.05)
      RadioObject:NewTransmission("DepartureFreq.ogg", 4.853, "Airbase_ATC/Clearance/SoundFiles/", nil, 0.05)
    ...
    elseif Procedure=="VFR" then
      subtitle = string.format("%s: Maintain at or below 4000 on departure. Contact departure on 385.4, Squawk %s", Title, squawk)..Sequential
      RadioObject:NewTransmission("DeadAir.ogg", 0.100, "Airbase_ATC/Ground/SoundFiles/", nil, nil, subtitle, 10)
      RadioObject:NewTransmission("VFRClearance.ogg", 5.863, "Airbase_ATC/Clearance/SoundFiles/", nil, 0.05)
      AI_ATC:ReadDigits(firstDigit, RadioObject, Transmitter)
      RadioObject:NewTransmission("ZeroZeroOne.ogg", 1.033, "Airbase_ATC/Clearance/SoundFiles/", nil, 0.05)
      if Flight then
        RadioObject:NewTransmission("Sequential.ogg", 0.600, "Airbase_ATC/Clearance/SoundFiles/", nil, 0.05)
...
function AI_ATC:ClearanceReadBack(Alias, code)
  ...
  if Squawk~=code then
    local Type = ClientData.DepartureCode
    AI_ATC:ClearanceDelivery(Alias, Type)
    return
  end
  ...
  local function Execute()
    SCHEDULER:New(nil, function()
      ...
      AI_ATC:ChannelOpen(6, Transmitter, Alias)
      local subtitle = string.format("%s: %s, Readback Correct, Contact Nellis Ground prior to Taxi.", Title, CallsignSub )
      RadioObject:NewTransmission("DeadAir.ogg", 0.100, "Airbase_ATC/Ground/SoundFiles/", nil, nil, subtitle, 6)
      AI_ATC:Callsign(Callsign, RadioObject, Transmitter, Flight)
      RadioObject:NewTransmission("ReadBack.ogg", 3.158, "Airbase_ATC/Clearance/SoundFiles/", nil, 0.1)
    end,{}, Delay )
  end
...
function AI_ATC:HoldShortTracker(Alias)
  ...
  if not HoldShortZones or #HoldShortZones == 0 then
    env.info(string.format("AI_ATC:HoldShortTracker - No HoldShort zones for '%s'.", Alias))
    return
  end
...
function AI_ATC:CrossRunway(Alias, Audio)
  ...
  local HoldShort2 = "03L"
  local HoldShortZones = HoldShortData[HoldShort2]
  if not HoldShortZones or #HoldShortZones == 0 then
    env.info(string.format("AI_ATC:HoldShortTracker - No HoldShort zones for '%s'.", HoldShort2))
    return
  end
  ...
  if AI_Wingman_1_Unit then
    ...
    for index, ZoneObject in ipairs(HoldShortZones) do
      if AI_Wingman_1_Unit then
        ZoneObject:SetRadius(50)
        local GuardZone = RunwayGuard[HoldShort2] and RunwayGuard[HoldShort2][index]
        if GuardZone then
          local GuardCoord = GuardZone:GetCoordinate()
          SCHEDULER:New(nil, function()
            AI_ATC:SpawnHoldShort(GuardCoord, index)
...
function AI_ATC:VFRDeparture(Alias, Audio)
  ...
  local SchedulerObjects = ClientData.SchedulerObjects
  local Callsign = ClientData.Callsign
  local VFRProcedure = ClientData.VFR
  local FlightCallsign = ClientData.FlightCallsign
  local Flight = ClientData.Flight
  local CallsignSub = Flight and FlightCallsign or Callsign
  local Delay = 1.5 + math.random() * (2.5 - 1.5)
  ...
  SCHEDULER:New(nil, function()
    if AI_Instructor then
      if AI_Instructor.Active == true then 
        SCHEDULER:New(nil, function()
          AI_Instructor.ContactDeparture = true
        end, {}, Delay + 10)
      end
    end
  ...
  local function TrafficReport()
    ...
    local TrafficCourse = tonumber(AI_ATC:CorrectHeading(Traffic:GetHeading()))
    local TrafficCardinal = UTILS.BearingToCardinal(TrafficCourse)
    local Range = tostring(math.floor(TrafficCoord:Get2DDistance(PlayerCoord) / 1852 + 0.5))
    local BearingHeading = tonumber(AI_ATC:CorrectHeading(PlayerCoord:HeadingTo(TrafficCoord)))
    local RelativeHeading = (BearingHeading - PlayerHeading) % 360
  ...
  if Audio~=true then
    SchedulerObject = SCHEDULER:New(nil, function()
      ...
      if FunctionDelay and ClassBZone and not Unit:IsInZone(ClassBZone) then
        AI_ATC:RadarTerminate(Alias)
        SchedulerObject:Stop()
        return
      end
      if FunctionDelay
         and VFRProcedure == "MORMON MESA"
         and MormonMesaZone
         and not Unit:IsInZone(MormonMesaZone)
         and not TurnPermit
      then
        ExecuteTurn()
      end
...
function AI_ATC:TrafficReport(Alias, RadioObject, Transmitter)
  ...
  UnitCoord = Unit:GetCoordinate()
  UnitHeading = tonumber(AI_ATC:CorrectHeading(Unit:GetHeading()))
  ...
  Radar:ForEachGroup(function(GroupObject)
    if not GroupObject:IsAirborne() then
      ...
      local BearingLocal = tonumber(AI_ATC:CorrectHeading(UnitCoord:HeadingTo(TrafficCoord)))
  ...
  local function ReportTraffic(Traffic)
    ...
    TrafficHeading = tonumber(AI_ATC:CorrectHeading(Traffic:GetHeading()))
    ...
    local BearingHeading = tonumber(AI_ATC:CorrectHeading(UnitCoord:HeadingTo(TrafficCoord)))
    local RelativeHeading = (BearingHeading - UnitHeading) % 360
    Bearing = tostring(AI_ATC:HeadingToClockBearing(RelativeHeading))
    local feet = Traffic:GetAltitude() / 0.3048
...
function AI_ATC:RadarTerminate(Alias, Audio)
  ...
  if Audio~=true then
    if AI_Instructor then
      if AI_Instructor.Active == true then 
        SCHEDULER:New(nil, function()
          AI_Instructor.RadarTerminate = true
        end, {}, Delay + 5)
      end
    end
    AI_ATC:TerminateSchedules(Alias)
...
function AI_ATC:RangeCheckIn(Alias, String, Audio)
  ...
  SCHEDULER:New(nil, function()
     if AI_Instructor then
      if AI_Instructor.Active == true then 
        SCHEDULER:New(nil, function()
          AI_Instructor.CheckedInGate3 = true
        end, {}, Delay + 7)
      end
    end
...
function AI_ATC:ContainerAdvisory(Alias)
  ...
  local function CalculateDeconfliction(Unit, Container)
    local UnitCoord = Unit:GetCoordinate()
    local UnitHeading = tonumber(AI_ATC:CorrectHeading(Unit:GetHeading()))
...
function AI_ATC:RangeCheckOut(Alias, String, Audio)
  ...
  if AI_Instructor and AI_Instructor.CheckoutRequest==true then
    AI_Instructor.CheckoutPremit = true
  end
...
function AI_ATC:NATCF_IFR(Alias, String)
  ...
  if AI_Instructor and AI_Instructor.NATCFRequest==true then
    AI_Instructor.JAYSNRecovery = true
  end
...
function AI_ATC:VFRApproach(Alias, Modifier)
  ...
  if AI_Instructor and AI_Instructor.ContactApproach==true then
    AI_Instructor.ApproachClearance = true
  end
...
function AI_ATC:ApproachManager(Alias, Type, Missed)
  ...
  AudioFile, AudioFile2, SchedulerObject, Instruction, ClassBVec2, ClassBZone, Zone_2, condition, audioKey
  local Height, WindDirection, WindSpeed, QNHPressure, QNH, Turn, Heading, TurnSub, TurnAudio
  local SchedulerObject3, UnitHdg, UnitCoord, HeadingDiff, Vector, OffsetHeading
  
  local function CalculateTurnDirection(unitobject, requiredheading)
  ...
  local function CorrectHeading(NewHeading)
    ...
    SCHEDULER:New(nil, function()
      AI_ATC:ChannelOpen(12, Transmitter, Alias)
      local Subtitle = string.format("%s: %s, %s Approach, fly heading %s.", Title, CallsignSub, Title, NewHeading)
      RadioObject:NewTransmission("DeadAir.ogg", 0.100, "Airbase_ATC/Ground/SoundFiles/", nil, 0.3, Subtitle, 5)
      AI_ATC:Callsign(Callsign, RadioObject, Transmitter, Flight)
      if Type~="PAR" then
        AI_ATC:AirbaseName(Airbase, RadioObject, Transmitter)
        ...
        RadioObject:NewTransmission("GCA2.ogg", 0.987, "Airbase_ATC/Departure/SoundFiles/", nil, 0.01)
      end
      RadioObject:NewTransmission("FlyHeading.ogg", 0.801,"Airbase_ATC/Departure/SoundFiles/", nil, 0.01)
  ...
  local function HeadingCorrection(Hdg)
    ...
    table.insert(ClientData.SchedulerObjects, SchedulerObject3)
  end
  
  local function GCAInstruction()
    if AI_ATC:FunctionDelay(Alias, function()  GCAInstruction() end, Transmitter)==false then return end
    if Missed then
      FinalCoord = AI_ATC_Navpoints["ARCOE"]:GetCoordinate()
      Altitude = "10"
      PatternAlt = 10000
      AltSub = "10000"
    end
    local UnitCoord = Unit:GetCoordinate()
    local PlayerHeading = tonumber(AI_ATC:CorrectHeading(Unit:GetHeading()))
    local FinalHeading = AI_ATC:CorrectHeading(UnitCoord:HeadingTo(FinalCoord)+0.5)
    Turn = CalculateTurnDirection(Unit, FinalHeading)
    TurnAudio = TurnTable[Turn]
    TurnSub = string.format("Turn %s heading %s", Turn, FinalHeading)
    CalculateAltitude()
    AI_ATC:RepeatLastTransmission(Alias, function() GCAInstruction() end)
    AI_ATC:ChannelOpen(8, Transmitter, Alias)

    local Subtitle = string.format("%s: %s, %s, %s ", Title, CallsignSub,  TurnSub, AltSubtitle )
    RadioObject:NewTransmission("DeadAir.ogg", 0.100, "Airbase_ATC/Ground/SoundFiles/", nil, 0.3, Subtitle, 4)
    AI_ATC:Callsign(Callsign, RadioObject, Transmitter, Flight)
    RadioObject:NewTransmission(TurnAudio.filename, TurnAudio.duration, "Airbase_ATC/Departure/SoundFiles/", nil, 0.1)
  ...
  local function Execute()
    local function Message()
      ...
      local Subtitle = string.format("%s: %s, %s, fly heading %s. %s. %s", Title, CallsignSub, Title, TurnSub, AltSubtitle, Instruction)
      RadioObject:NewTransmission("DeadAir.ogg", 0.100, "Airbase_ATC/Ground/SoundFiles/", nil, 0.3, Subtitle, 7)
      AI_ATC:Callsign(Callsign, RadioObject, Transmitter, Flight)
      AI_ATC:AirbaseName(Airbase, RadioObject, Transmitter)
      if Type~="PAR" then
    ...
    local function AlternateMessage()
      ...
      local RadioObject = AI_ATC:FindTransmitter(Alias, Transmitter)
      AI_ATC:ChannelOpen(8, Transmitter, Alias)
      local Subtitle = string.format("%s: %s, %s. %s.", Title, CallsignSub, Title, Instruction)
      RadioObject:NewTransmission("DeadAir.ogg", 0.100, "Airbase_ATC/Ground/SoundFiles/", nil, 0.3, Subtitle, 7)
      AI_ATC:Callsign(Callsign, RadioObject, Transmitter, Flight)
      if Type~="PAR" then
        AI_ATC:AirbaseName(Airbase, RadioObject, Transmitter)
...
function AI_ATC:ApproachLocalizer(Alias, Type)
  ...
  local function CalculateTurnDirection(unitObject, desiredHeading)
    local UnitHeading = AI_ATC:CorrectHeading(unitObject:GetHeading())
    ...
    if not desiredHeading then
      ...
      if vectorCalc > 0 then
        offsetHeading = LOCALISER - 29
      else
        offsetHeading = LOCALISER + 31
      end
    else
      offsetHeading = tonumber(desiredHeading)
    end
  
    UnitHeading = UnitHeading % 360
    offsetHeading = offsetHeading % 360
  
    local headingError = ((offsetHeading - UnitHeading + 180) % 360) - 180
    ...
    return turnDirection, tostring(offsetHeading)
  end
  ...
  local function Execute()
    SCHEDULER:New(nil, function()
      UnitCoord = Unit:GetCoordinate()
      Turn, Heading = CalculateTurnDirection(Unit)
      AudioFile = TurnTable[Turn]
      ...
      if Type=="ILS" then
        ...
        Altitude = "4.2"
        AltitudeSub = "4200"
        ApproachSub = string.format("established on the localizer. Cleared ILS approach for runway %s.", Runway)
...
function AI_ATC:ApproachBackToRadar(Alias, Type)
  ...
  AI_ATC:TerminateSchedules(Alias)
  AI_ATC:ResetMenus(Alias)
  AI_ATC:GenerateEmptyMenu(Alias, Transmitter, 3)
  AI_ATC:CancelIFRSubMenu(Alias)
  USERSOUND:New("RADIO_TRANS_START.ogg"):ToUnit(Unit)
...
function AI_ATC:RadarManager(Alias, Type, Auto)
  ...
  local ApproachTypeFile, Localizer
  ...
  local function TurnInstruction(heading)
    ...
    local UnitCoord = Unit:GetCoordinate()
    Turn = CalculateTurnDirection(Unit, Heading)
  ...
  local function TurnThree()
    ...
    SCHEDULER:New(nil, function()
      local Turn, TurnAudioFile, TurnSub
      RadioObject = AI_ATC:FindTransmitter(Alias, Transmitter)
...
function AI_ATC:RadarManager2(Alias, Type)
  ...
  local Airbase, Runway, AirbaseCoord, QNHPressure, QNH, Height, WindDirection, WindSpeed
  ...
  SCHEDULER:New(nil, function()
    ...
    Airbase = AI_ATC.Airbase
    LSVTCN = AI_ATC_Navpoints.LSVTCN:GetCoordinate()
    AirbaseCoord = AI_ATC_Airbase:GetCoordinate()
    Height = AirbaseCoord:GetLandHeight()
    WindDirection, WindSpeed = AirbaseCoord:GetWind(Height +10)
    WindDirection = AI_ATC:RectifyHeading(tostring(math.floor(WindDirection + 0.5)))
    WindSpeed = (tostring(math.floor(UTILS.MpsToKnots(WindSpeed)- 0.5)))
  ...
  local function Execute()
    local function Audio()
      ...
      SCHEDULER:New(nil, function()
        AI_ATC:ChannelOpen(12, Transmitter, Alias)
        local Subtitle = string.format("%s: %s, %s, fly heading %s", Title, CallsignSub, Title, Heading)
        RadioObject:NewTransmission("DeadAir.ogg", 0.100, "Airbase_ATC/Ground/SoundFiles/", nil, 0.3, Subtitle, 5)
        AI_ATC:Callsign(Callsign, RadioObject, Transmitter, Flight)
        if Type~="PAR" then
          AI_ATC:AirbaseName(Airbase, RadioObject, Transmitter)
  ...
  local function GCAExecute()
    SCHEDULER:New(nil, function()
      ...
      RadioObject:NewTransmission("At.ogg", 0.177, "Airbase_ATC/Departure/SoundFiles/", nil, nil)
      AI_ATC:ReadNumber(WindSpeed, RadioObject, Transmitter)
      RadioObject:NewTransmission("Altimeter.ogg", 0.708, "Airbase_ATC/Departure/SoundFiles/", nil, 0.3)
      AI_ATC:Pressure(QNH, RadioObject, Transmitter)
      
      local Subtitle = string.format("%s: %s, fly heading %s Maintain at or above %s.", Title, CallsignSub, Heading, AltSub)
      RadioObject:NewTransmission("DeadAir.ogg", 0.100, "Airbase_ATC/Ground/SoundFiles/", nil, 0.3, Subtitle, 6)
      AI_ATC:Callsign(Callsign, RadioObject, Transmitter, Flight)
      RadioObject:NewTransmission("FlyHeading.ogg", 0.801, "Airbase_ATC/Departure/SoundFiles/", nil, 0.01)
      AI_ATC:ReadHeading(Heading, RadioObject, Transmitter)
      RadioObject:NewTransmission("AtOrAbove.ogg", 1.289, "Airbase_ATC/Departure/SoundFiles/", nil, 0.1, nil)
      AI_ATC:ReadFlightLevel(Altitude, RadioObject, Transmitter, true)
    end, {}, 1)
...
function AI_ATC:Push_Tower(Alias)
  ...
  local function Execute()
    if AI_Instructor then
      AI_Instructor.TowerHandOff = true
    end
    local Procedure = AI_ATC.Procedure
    ...
    if Approach=="IFR" and Procedure=="VFR" then
      AI_ATC:GenerateEmptyMenu(Alias, "Approach", 3)
      AI_ATC:CancelIFRSubMenu(Alias)
    end
    ...
    SCHEDULER:New(nil, function()
      ...
      elseif Approach=="IFR" and Type~="PAR" then
      ...
      elseif Approach=="IFR" and Type=="PAR" then
        local Subtitle = string.format("%s: %s contact final controller on UHF Channel 4.", Title, CallsignSub)
        RadioObject:NewTransmission("DeadAir.ogg", 0.100, "Airbase_ATC/Ground/SoundFiles/", nil, 0.3, Subtitle, 3)
        AI_ATC:Callsign(Callsign, RadioObject, Transmitter, Flight)
        RadioObject:NewTransmission("FinalController.ogg", 2.752, "Airbase_ATC/Departure/SoundFiles/", nil, 0.01)
      end
...
function AI_ATC:CancelIFR(Alias, agency)
  ...
  local function TowerMessage()
    SCHEDULER:New(nil, function()
      ...
      local Subtitle = string.format("%s: %s Contact Nellis Tower on UHF Channel 4.", Title, CallsignSub)
      RadioObject:NewTransmission("DeadAir.ogg", 0.100, "Airbase_ATC/Ground/SoundFiles/", nil, 0.3, Subtitle, 3)
      AI_ATC:Callsign(Callsign, RadioObject, Agency, Flight)
...
function AI_ATC:AcknowledgeRunway(Alias)
  ...
  local function RunwayInsight()
    SCHEDULER:New(nil, function()
      ...
      RadioObject:NewTransmission("DeadAir.ogg", 0.100, "Airbase_ATC/Ground/SoundFiles/", nil, 0.3, Subtitle, 4)
      AI_ATC:Callsign(Callsign, RadioObject, Agency, Flight)
      RadioObject:NewTransmission("RunwayInsight.ogg", 1.186, "Airbase_ATC/Ground/SoundFiles/", nil, 0.1)
    end, {}, Delay)
  end
...
function AI_ATC:Report5MileInitial(Alias)
  ...
  local AirbaseCoord, RunwayHeading, ReciprocalHeading, UnitCoord, InitRange, Break, LandingNo, LandingType, SchedulerObject, SchedulerObject2
  ...
  local function Message()
    SCHEDULER:New(nil, function()
      if AI_Instructor then
        AI_Instructor.LeftBreak = true
      end
      SCHEDULER:New(nil, function()
        AI_ATC:ProvideInstruction(Alias, "Tower", "Base")
      end, {}, Delay + 10)
      AI_ATC:RepeatLastTransmission(Alias, function()Message() end)
      AI_ATC:ChannelOpen(12, Transmitter, Alias)
...
function AI_ATC:ReportInitial(Alias)
  ...
  local function Message()
    SCHEDULER:New(nil, function()
      AI_ATC:RepeatLastTransmission(Alias, function()Message() end)
      AI_ATC:ChannelOpen(12, Transmitter, Alias)
      local Subtitle = string.format("%s: %s, check gear down, Runway %s. Wind %s at %s. ", Title, Callsign, Runway, WindDirection, WindSpeed)
...
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--****************************************************************************ATC INSTRUCTOR INITIAL*****************************************************************************--
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
function AI_ATC:InstructorBaseTurn(Alias, Callsign)
  local Transmitter = "Tower"
  local RadioObject = TOWER_RADIO
  local Title = string.format("%s %s", AI_ATC.Airbase, Transmitter)
  local Callsign = Callsign:sub(1, -2) .. "1"
  local CallsignSub = Callsign
  local Delay = math.random(1.5, 2.5)
...
function AI_ATC:BreakOut(Alias, NavPoint)
  ...
  local AirbaseCoord, Instruction
  ...
  SCHEDULER:New(nil, function()
    ...
    Instruction = string.format("Break Out. Re-Enter via %s. Report %s", NavPoint, NavPoint)
  ...
  local function RadioMessage()
    SCHEDULER:New(nil, function()
      ...
      RadioObject:NewTransmission("DeadAir.ogg", 0.100, "Airbase_ATC/Ground/SoundFiles/", nil, 0.3, Subtitle, 5)
      AI_ATC:Callsign(Callsign, RadioObject, Transmitter, Flight)
      if NavPoint == "DUCK" then
        RadioObject:NewTransmission("EnterDuck.ogg", 1.812, "Airbase_ATC/Ground/SoundFiles/", nil, 0.03)
        RadioObject:NewTransmission("ReportDuck.ogg", 0.740, "Airbase_ATC/Ground/SoundFiles/", nil, 0.3)
      else
        RadioObject:NewTransmission("EnterFlex.ogg", 2.050, "Airbase_ATC/Ground/SoundFiles/", nil, 0.03)
        RadioObject:NewTransmission("ReportFlex.ogg", 0.816, "Airbase_ATC/Ground/SoundFiles/", nil, 0.3)
      end
    end, {}, Delay)
  end
...
function AI_ATC:GoAround(Alias, Type)
  ...
  SCHEDULER:New(nil, function()
    ...
    elseif finalApproach == "IFR" then
      ...
      if ApproachChart=="PAR" then
        ATM.ClientData[Alias].Chart = "PAR"
      elseif ApproachChart == "HI-ILS" or ApproachChart == "ILS" then
        ATM.ClientData[Alias].Chart = "ILS"
      else
...
function AI_ATC:TowerPAR(Alias)
  ...
  local SchedulerCount, DepartureZone, TouchDownPoint, RunwayHeading, DecisionHeight, Bearing
  ...
  local GlidepathCallout, CourseCallout, InterceptHeading, CourseStatus, AltAdvisoryFlag
  ...
  SCHEDULER:New(nil, function()
    ...
    if Runway=="21L" then
      DepartureZone = AI_ATC_Navpoints["03R"]:GetCoordinate()
      TouchDownPoint = AI_ATC_Navpoints["21L"]:GetCoordinate()
    else
      DepartureZone = AI_ATC_Navpoints["03L"]:GetCoordinate()
      TouchDownPoint = AI_ATC_Navpoints["21R"]:GetCoordinate()
    end
  ...
  local function RunwayInSight()
    local Subtitle = string.format("%s: Copy Runway in sight.", Title)
  ...
  local function AltAdvisory()
    ...
    AI_ATC:ReadFlightLevel("4.2", RadioObject, Agency, true, 0.00)
  end
...
function AI_ATC:TaxiParking(Alias)
  ...
  local Count = 0
  local Iteration = 10
  local Delay = 1.0 + math.random() * (2.5 - 1.0)
  local Callsign = ClientData.Callsign
  local FlightCallsign = ClientData.FlightCallsign
  local Flight = ClientData.Flight
  local CallsignSub = Flight and FlightCallsign or Callsign
  ...
  SCHEDULER:New(nil, function()
    ...
    if AI_Instructor then
      if HoldShort==Runway then
        AI_Instructor.TaxiToParking = 1
      elseif HoldShort~=Runway then
        AI_Instructor.TaxiToParking = 2
      end
    end
  end, {}, 0.5)
...
function AI_ATC:CrossRunwayLanding(Alias, Audio)
  ...
  if Audio~=true then
    ...
    AI_ATC:TaxiSubMenu(Alias)
    if AI_Instructor then
      AI_Instructor.ClearanceToCross = true
    end
  end
